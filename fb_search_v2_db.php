...
<div id="content">
	<form id = "search_box" method="post" action="" name="search" align = "center">
		...
		<select id = "search_select" name = 'method'>
...
<?php
	if (isset($_POST['submit'])) {
		...
		//SQL 수행
		if ($method == 'type_name')
			$query = "SELECT * FROM fb_202111729 WHERE name like '%$search%'";
		else if ($method == 'type_email')
			$query = "SELECT * FROM fb_202111729 WHERE email like '%$search%'";
		else
			$query = "SELECT * FROM fb_202111729 WHERE phone like '%$search%'";
